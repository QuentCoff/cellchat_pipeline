#!/usr/bin/env Rscript
# 09_bubble_dysfunctional.R (SSC_Sertoli)
# CellChat Procedure 2 Step 12: Identify dysfunctional signaling
# Option A: bubble plots comparing communication probabilities
# Option B: differential expression analysis
#
# Usage:
#   Rscript 09_bubble_dysfunctional.R <prefix1> <prefix2>
#
# Example:
#   Rscript 09_bubble_dysfunctional.R healthy crypto
...
if (length(args) < 2) {
  stop("Usage: Rscript 09_bubble_dysfunctional.R <prefix1> <prefix2>\n")
}
...
out_dir <- file.path(base_dir, "results", "SSC_Sertoli", "Procedure_2",
                     paste0(prefix1, "_vs_", prefix2), "comparison", "step12")
...
# Cell-type indices for Sertoli ↔ SSC analysis
idents <- levels(cellchat@idents$joint)
sertoli_idx <- match("Sertoli", idents)
ssc_idx     <- match("SSC",     idents)

if (any(is.na(c(sertoli_idx, ssc_idx)))) {
  stop("One or more cell types (Sertoli, SSC) not found in idents\n")
}
...
cat("\n--- Procedure 2 Step 12 complete (SSC_Sertoli) ---\n")
