#!/usr/bin/env Rscript
# 06_signaling_role_scatter.R (SSC_Sertoli)
...
out_dir <- file.path(base_dir, "results", "SSC_Sertoli", "Procedure_2",
                     paste0(prefix1, "_vs_", prefix2), "comparison", "step9")
...
cell_types <- c("Sertoli", "SSC")

for (ct in cell_types) {
  cat(paste0("  Processing: ", ct, "\n"))
  if (!(ct %in% levels(cellchat_b@idents$joint))) {
    cat(paste0("    Skipped: '", ct, "' not present in merged idents.\n"))
    next
  }
  gg_b <- tryCatch(
    netAnalysis_signalingChanges_scatter(cellchat_b, idents.use = ct),
    error = function(e) {
      cat(paste0("    Error: ", conditionMessage(e), "\n"))
      NULL
    }
  )
  if (is.null(gg_b)) next
  ggplot2::ggsave(
...
cat("--- Procedure 2 Step 9 complete (SSC_Sertoli) ---\n")
