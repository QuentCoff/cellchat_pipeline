#!/usr/bin/env Rscript
# CellChat analysis - Crypto samples only
...
# Filtrer sur les Bi-crypto uniquement
cat("\n=== Filtrage sur Bi-crypto ===\n")
cells_crypto <- colnames(seurat_obj)[seurat_obj$detailed_group == "Bi-Crypto"]
seurat_crypto <- subset(seurat_obj, cells = cells_crypto)

cat(paste0("Cellules Crypto: ", ncol(seurat_crypto), "\n"))
cat("Cell types dans Crypto:\n")
print(table(seurat_crypto$cell_type))
...
data.input <- GetAssayData(seurat_crypto, assay = "RNA", layer = "data")
...
labels <- seurat_crypto$cell_type
...
out_dir <- "/LAB-DATA/GLiCID/users/e244507t@univ-nantes.fr/Cellchat/results/crypto/interactions"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# Réseau agrégé
png(file.path(out_dir, "network_count_crypto.png"), width = 1000, height = 800)
...
png(file.path(out_dir, "network_weight_crypto.png"), width = 1000, height = 800)
...
png(file.path(out_dir, "network_weight_separated_crypto.png"), width = 1500, height = 1200)
...
pathway_dir <- "/LAB-DATA/GLiCID/users/e244507t@univ-nantes.fr/Cellchat/results/crypto/pathway"
...
p_bubble <- ggplot2::ggplot(df_bub, ggplot2::aes(x = celltype, y = pathway,
  ...
  ggplot2::labs(x = NULL, y = NULL, size = "Outgoing\nstrength",
                title = "Outgoing signaling - Crypto") +
...
report_file <- file.path(out_dir, "statistiques_crypto.txt")
sink(report_file)

cat("========================================\n")
cat("RAPPORT STATISTIQUES - CRYPTO\n")
...
# 2. Statistiques Crypto
cat("2. STATISTIQUES BI-CRYPTO\n")
cat("--------------------------\n")
cat(paste0("Cellules Bi-crypto: ", ncol(seurat_crypto), "\n\n"))

cat("Cell types dans Bi-crypto:\n")
cell_counts <- table(seurat_crypto$cell_type)
print(cell_counts)
cat("\n")
...
seurat_crypto$nGene <- Matrix::colSums(GetAssayData(seurat_crypto, assay = "RNA", layer = "counts") > 0)

# Moyenne par cluster
avg_genes_per_cluster <- tapply(seurat_crypto$nGene, seurat_crypto$cell_type, mean)
...
rds_file <- "/LAB-DATA/GLiCID/users/e244507t@univ-nantes.fr/Cellchat/results/crypto/cellchat_crypto.rds"
