#!/usr/bin/env Rscript
# CellChat analysis - Descended samples only
...
# Filtrer sur les Healthy uniquement (Descended dans detailed_group)
cat("\n=== Filtrage sur Healthy (Descended) ===\n")
cells_descended <- colnames(seurat_obj)[seurat_obj$detailed_group == "Healthy"]
seurat_descended <- subset(seurat_obj, cells = cells_descended)

cat(paste0("Cellules Descended: ", ncol(seurat_descended), "\n"))
cat("Cell types dans Descended:\n")
print(table(seurat_descended$cell_type))

# ============================================
# 2. CREATION DE L'OBJET CELLCHAT (Option B: depuis Seurat)
# ============================================

cat("\n=== Création CellChat ===\n")

# Créer l'objet CellChat directement depuis l'objet Seurat (Option B du protocole)
cellchat <- createCellChat(object = seurat_descended, group.by = "cell_type", assay = "RNA")

# Afficher les dimensions
cat(paste0("Dimensions données: ", nrow(cellchat@data.signaling), " gènes x ", ncol(cellchat@data.signaling), " cellules\n"))
cat(paste0("Nombre de groupes cellulaires: ", length(levels(cellchat@idents)), "\n"))
...
# Inspection de la base de données (Step 4 du protocole)
...
# Activer le parallélisme multicœur (Step 7 du protocole)
...
cat("\n=== Inférence du réseau ===\n")
cellchat <- computeCommunProb(cellchat, type = "triMean", trim = NULL, raw.use = TRUE, population.size = TRUE)
...
out_dir <- "/LAB-DATA/GLiCID/users/e244507t@univ-nantes.fr/Cellchat/results/descended/interactions"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# Réseau agrégé
png(file.path(out_dir, "network_count_descended.png"), width = 1000, height = 800)
...
png(file.path(out_dir, "network_weight_descended.png"), width = 1000, height = 800)
...
png(file.path(out_dir, "network_weight_separated_descended.png"), width = 1500, height = 1200)
...
pathway_dir <- "/LAB-DATA/GLiCID/users/e244507t@univ-nantes.fr/Cellchat/results/descended/pathway"
...
# Identifier les patterns de communication sortante (NMF)
...
p_bubble <- ggplot2::ggplot(df_bub, ggplot2::aes(x = celltype, y = pathway,
  ...
  ggplot2::labs(x = NULL, y = NULL, size = "Outgoing\nstrength",
                title = "Outgoing signaling - Descended") +
...
# Diagramme alluvial (river plot) - utilise automatiquement cellchat@idents$colors
...
report_file <- file.path(out_dir, "statistiques_descended.txt")
sink(report_file)

cat("========================================\n")
cat("RAPPORT STATISTIQUES - DESCENDED\n")
...
# 2. Statistiques Descended
cat("2. STATISTIQUES HEALTHY (DESCENDED)\n")
cat("-------------------------------------\n")
cat(paste0("Cellules Healthy (Descended): ", ncol(seurat_descended), "\n\n"))

cat("Cell types dans Healthy (Descended):\n")
cell_counts <- table(seurat_descended$cell_type)
print(cell_counts)
cat("\n")
...
seurat_descended$nGene <- Matrix::colSums(GetAssayData(seurat_descended, assay = "RNA", layer = "counts") > 0)

# Moyenne par cluster
avg_genes_per_cluster <- tapply(seurat_descended$nGene, seurat_descended$cell_type, mean)
...
rds_file <- "/LAB-DATA/GLiCID/users/e244507t@univ-nantes.fr/Cellchat/results/Procedure_1/healthy/cellchat_descended.rds"
