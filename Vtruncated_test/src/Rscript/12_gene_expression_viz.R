...
rdata_deg <- file.path(base_dir, "Vtruncated_test", "Result", "plot",
...
for (pw in pathways.show) {
  ...
  tryCatch({
    ...
    pw_dir <- file.path(out_dir, pw)
    dir.create(pw_dir, showWarnings = FALSE, recursive = TRUE)

    ggplot2::ggsave(
      filename = file.path(pw_dir, paste0("violin_", pw, ".png")),
      plot     = gg_violin,
      width    = 14,
      height   = 10,
      dpi      = 150,
    ...
    cat(paste0("    Saved: ", pw, "/violin_", pw, ".png\n"))
  }, error = function(e) {
