...
#
# What it does:
#   Loads the multi-group object list and draws circle plots for each signaling pathway
#   across all conditions. If pathway names are passed as arguments, only those pathways
#   are visualized; otherwise all significant pathways are processed.
#
# Inputs:
...
#
# Outputs:
#   - One circle plot per pathway in Result/plot/<MERGED_DIR_NAME>/step14/:
#       circle_<pathway>.png
#
# Previous step: 01_merge_cellchat.R
# Next step: 12_gene_expression_viz.R
#
# Usage:
...
load(rdata_list)  # loads 'object.list' (one object per condition)
...
# Resolve pathway list: user-specified OR union of all significant pathways across datasets.
...
for (pw in pathways.show) {
  ...
  # Determine which datasets actually contain the pathway to avoid empty plots.
  ...
  tryCatch({
    # Compute max weight across datasets that have the pathway for a common scale.
    obj_subset <- object.list[idx_valid]
    weight.max <- getMaxWeight(obj_subset, slot.name = c("netP"), attribute = pw)

    # Choose figure layout based on how many datasets contain the pathway.
    ...
    # Open a PNG with one panel per dataset containing the pathway.
    ...
    # Draw the pathway-specific circle network for each valid dataset.
    for (i in idx_valid) {
      netVisual_aggregate(
        object.list[[i]],
        signaling        = pw,
      ...
      # Draw a white rectangle behind the title area to hide overlapping edges.
      par(xpd = TRUE)
      usr <- par("usr")
